...
router.get('/:id/read', isAuthenticated, async (req: Request, res: Response) => {
    try {
        const [books] = await pool.execute<RowDataPacket[]>(
        ...
        const template = book.file_type === 'pdf' ? 'books/reader-pdf' : 'books/reader-epub';
...
router.post('/:id/progress', isAuthenticated, async (req: Request, res: Response) => {
    try {
        ...
        await pool.execute(
            ...
            [userId, bookId, current_page || 0, current_location || null, current_page || 0, current_location || null]
        );
